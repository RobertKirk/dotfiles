    ...
    if #chrome_running > 0 then
        ...
        if stat then
            for idx,val in pairs(data) do
                ...
                table.insert(chooser_data, {text=val[1], subText=val[2], image=hs.image.imageFromPath(obj.spoonPath .. "/resources/chrome.png"), output="focustab", arg=idx})
            end
        end
    end
...
local function focusTab(arg)
    hs.osascript.applescript('tell application "Google Chrome" to set active tab index of first window to ' .. arg)
